...
export default function InteractiveDashboard({ lang }: InteractiveDashboardProps) {
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in" id="lab_dashboard">
        ...
        <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-zinc-900/60 border border-zinc-800 p-5 rounded-2xl relative overflow-hidden" id="lab_kpi_1">
            <div className="absolute top-0 right-0 w-24 h-24 bg-teal-500/10 rounded-full blur-2xl"></div>
            <p className="text-sm text-zinc-400">{t.utilizationRate[lang]}</p>
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-4xl font-bold text-teal-400 font-mono">{avgUtil}%</span>
              <span className="text-xs text-emerald-400">↑ 25% {lang === 'en' ? 'from 70%' : 'من الأساسي 70%'}</span>
            </div>
            <div className="w-full bg-zinc-800 h-2 rounded-full mt-4 overflow-hidden">
              <div className="bg-gradient-to-r from-teal-500 to-cyan-400 h-full rounded-full transition-all duration-1000" style={{ width: `${avgUtil}%` }}></div>
            </div>
          </div>

          <div className="bg-zinc-900/60 border border-zinc-800 p-5 rounded-2xl relative overflow-hidden" id="lab_kpi_2">
            <div className="absolute top-0 right-0 w-24 h-24 bg-cyan-500/10 rounded-full blur-2xl"></div>
            <p className="text-sm text-zinc-400">{t.testsConducted[lang]}</p>
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-4xl font-bold text-cyan-400 font-mono">758</span>
              <span className="text-xs text-zinc-400">{lang === 'en' ? 'Active This Month' : 'نشط هذا الشهر'}</span>
            </div>
            <div className="flex gap-2 mt-4 text-xs text-zinc-400">
          ...
          <div className="bg-zinc-900/60 border border-zinc-800 p-5 rounded-2xl relative overflow-hidden" id="lab_kpi_3">
            <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-500/10 rounded-full blur-2xl"></div>
            <p className="text-sm text-zinc-400">{t.bottleneckIndex[lang]}</p>
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-4xl font-bold text-indigo-400 font-mono">1.2x</span>
              <span className="text-xs text-emerald-400">↓ {lang === 'en' ? 'Optimized' : 'تم تحسينه'} (was 2.8x)</span>
            </div>
        ...
        {/* Charts */}
        <div className="lg:col-span-2 bg-zinc-900/40 border border-zinc-800/80 p-5 rounded-2xl flex flex-col justify-between" id="lab_chart_main">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
              <h4 className="text-base font-semibold text-zinc-200">{lang === 'en' ? 'Historical Utilization Trend' : 'منحنى الاستغلال التاريخي للمختبر'}</h4>
              <p className="text-xs text-zinc-400">{lang === 'en' ? 'Continuous tracking post-reengineering' : 'تتبع مستمر بعد إعادة هندسة الجدولة'}</p>
            </div>
            {/* Filter buttons */}
            <div className="flex flex-wrap gap-1 bg-zinc-950 p-1 rounded-lg border border-zinc-800 text-xs">
              <button 
                onClick={() => setLabFilter('all')} 
                className={`px-3 py-1.5 rounded-md transition-all ${labFilter === 'all' ? 'bg-teal-500 text-black font-semibold' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                {t.filterAll[lang]}
              </button>
              <button 
                onClick={() => setLabFilter('mechanical')} 
                className={`px-3 py-1.5 rounded-md transition-all ${labFilter === 'mechanical' ? 'bg-teal-500 text-black font-semibold' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                {t.filterMech[lang]}
              </button>
              <button 
                onClick={() => setLabFilter('electrical')} 
                className={`px-3 py-1.5 rounded-md transition-all ${labFilter === 'electrical' ? 'bg-teal-500 text-black font-semibold' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                {t.filterElec[lang]}
              </button>
            </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={labData.utilizationTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                {labFilter === 'all' || labFilter === 'mechanical' ? (
                  <Area type="monotone" name={lang === 'en' ? 'Mechanical Lab' : 'المختبر الميكانيكي'} dataKey="mechanical" stroke="#14b8a6" strokeWidth={2} fillOpacity={1} fill="url(#colorUtil)" />
                ) : null}
                {labFilter === 'all' || labFilter === 'electrical' ? (
                  <Area type="monotone" name={lang === 'en' ? 'Electrical Lab' : 'المختبر الكهربائي'} dataKey="electrical" stroke="#06b6d4" strokeWidth={2} fillOpacity={1} fill="url(#colorElec)" />
                ) : null}
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="bg-zinc-900/40 border border-zinc-800/80 p-5 rounded-2xl flex flex-col justify-between" id="lab_chart_downtime">
          <div>
            <h4 className="text-base font-semibold text-zinc-200">{t.downtimeCauses[lang]}</h4>
            <p className="text-xs text-zinc-400 mb-4">{lang === 'en' ? 'Key contributors to machine non-running hours' : 'أبرز العوامل المساهمة في توقف المعدات عن العمل'}</p>
          </div>
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in" id="reliability_dashboard">
        ...
        <div className="space-y-4 lg:col-span-1">
          <div className="bg-zinc-900/60 border border-zinc-800 p-5 rounded-2xl relative overflow-hidden" id="rel_sensor_1">
            <p className="text-xs text-zinc-400">{t.sensorTemp[lang]}</p>
            <div className="flex items-center justify-between mt-2">
              <span className={`text-5xl font-mono font-bold tracking-tight transition-colors duration-300 ${hasAnomalyNow ? 'text-rose-500' : 'text-teal-400'}`}>
                {currentTemp}°C
              </span>
              <div className="flex flex-col items-end">
                <span className="text-xs px-2 py-1 bg-zinc-800 rounded text-zinc-300 font-mono">Target: 4.0°C</span>
            ...
            <div className="mt-4 flex items-center justify-between border-t border-zinc-800 pt-3">
              ...
              <button 
                ...
                className="text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-1.5 rounded-lg flex items-center gap-1 transition-all"
              >
          ...
          <div className="bg-zinc-900/60 border border-zinc-800 p-4 rounded-2xl" id="rel_sensor_2">
            <p className="text-xs text-zinc-400">{t.sensorStatus[lang]}</p>
            <div className="mt-2 flex items-center gap-3">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${hasAnomalyNow ? 'bg-rose-950 text-rose-400 border border-rose-800' : 'bg-teal-950 text-teal-400 border border-teal-800'}`}>
          ...
          {/* Anomaly Logs */}
          <div className="bg-zinc-900/40 border border-zinc-800/80 p-4 rounded-2xl h-44 flex flex-col" id="rel_anomalies_log">
            <h5 className="text-xs font-bold text-zinc-400 mb-2 uppercase tracking-wider">{t.realtimeAlerts[lang]}</h5>
            <div className="overflow-y-auto space-y-2 flex-1 text-[11px] font-mono scrollbar-thin">
              <div className="text-zinc-500 border-b border-zinc-800 pb-1 flex justify-between">
              ...
              {tempData.filter(d => d.anomaly).map((d, i) => (
                <div key={i} className="text-rose-400 border-b border-zinc-800/50 pb-1 flex justify-between animate-pulse">
        ...
        <div className="lg:col-span-2 bg-zinc-900/40 border border-zinc-800/80 p-5 rounded-2xl flex flex-col justify-between" id="rel_line_chart_container">
          <div>
            <h4 className="text-base font-semibold text-zinc-200">{lang === 'en' ? 'Live Temperature & Humidity Telemetry' : 'بيانات المراقبة المباشرة للحرارة والرطوبة'}</h4>
            <p className="text-xs text-zinc-400 mb-4">{lang === 'en' ? 'Simulation of continuous sensor capture with automatic thresholds check' : 'محاكاة للقراءة المستمرة للمستشعرات مع رصد ذكي للمستويات المسموحة'}</p>
          </div>
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={tempData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <YAxis yAxisId="left" stroke="#14b8a6" fontSize={11} tickLine={false} />
                ...
                <Line 
                  ...
                  dot={(props: any) => {
                    ...
                    return <circle cx={cx} cy={cy} r={3} fill="#14b8a6" stroke="none" />;
                  }}
                  activeDot={{ r: 8 }} 
                />
                <Line 
  ...
  return (
    <div className="mt-16 bg-zinc-950 border border-zinc-800/80 p-6 md:p-8 rounded-3xl relative overflow-hidden" id="live_analytics_showcase">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs uppercase tracking-widest text-teal-400 font-mono font-semibold">{lang === 'en' ? 'Interactive Demonstration' : 'عرض تفاعلي حي'}</span>
          </div>
          <h3 className="text-2xl md:text-3xl font-bold tracking-tight text-white">{t.title[lang]}</h3>
          <p className="text-sm text-zinc-400 mt-1 max-w-2xl">{t.subtitle[lang]}</p>
        </div>

        {/* Tab triggers */}
        <div className="flex bg-zinc-900 border border-zinc-800 p-1 rounded-xl text-sm self-stretch md:self-auto">
          <button
            onClick={() => setActiveTab('lab')}
            className={`flex-1 md:flex-initial px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-all duration-200 ${activeTab === 'lab' ? 'bg-zinc-800 text-teal-400 font-medium shadow-sm' : 'text-zinc-400 hover:text-zinc-200'}`}
          >
            <BarChart2 size={16} />
          ...
          <button
            ...
            className={`flex-1 md:flex-initial px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-all duration-200 ${activeTab === 'reliability' ? 'bg-zinc-800 text-teal-400 font-medium shadow-sm' : 'text-zinc-400 hover:text-zinc-200'}`}
          >
            <Cpu size={16} />
