...
export default function Services({ data, lang }: ServicesProps) {
  ...
  const getServiceIcon = (iconName: string) => {
    switch (iconName) {
      case 'Binary':
        return <Binary className="text-teal-400" size={24} />;
      case 'LayoutDashboard':
        return <LayoutDashboard className="text-teal-400" size={24} />;
      case 'Zap':
        return <Zap className="text-teal-400" size={24} />;
      case 'ShieldAlert':
        return <ShieldAlert className="text-teal-400" size={24} />;
      default:
        return <CheckCircle2 className="text-teal-400" size={24} />;
    }
  };

  return (
    <section id="services" className="py-20 border-t border-zinc-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="services_grid">
          {data.services.map((service) => (
            <div
              key={service.id}
              className="bg-zinc-900/10 border border-zinc-800/60 p-6 sm:p-8 rounded-2xl hover:border-teal-500/30 hover:bg-zinc-900/30 transition-all duration-300 relative group overflow-hidden"
            >
              ...
              <div className="flex items-start gap-4 mb-6 relative z-10">
                <div className="h-12 w-12 rounded-xl bg-zinc-950 border border-zinc-850 flex items-center justify-center text-teal-400 shadow-md group-hover:border-teal-500/20 transition-all shrink-0">
              ...
              <ul className="space-y-3 relative z-10 text-xs sm:text-sm">
                {service.items.map((point, idx) => (
                  <li key={idx} className="flex items-start gap-2.5 text-zinc-400 group-hover:text-zinc-300 transition-colors font-sans">
