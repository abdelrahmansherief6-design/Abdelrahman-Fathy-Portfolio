...
export default function About({ data, lang }: AboutProps) {
  ...
  const getWhyIcon = (iconName: string) => {
    switch (iconName) {
      case 'Factory':
        return <Factory className="text-teal-400" size={20} />;
      case 'Award':
        return <ShieldCheck className="text-teal-400" size={20} />;
      case 'GitMerge':
        return <GitMerge className="text-teal-400" size={20} />;
      case 'TrendingUp':
        return <Zap className="text-teal-400" size={20} />;
      default:
        return <CheckCircle2 className="text-teal-400" size={20} />;
    }
  };
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start" id="about_main">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="text-zinc-400 text-sm leading-relaxed space-y-4 font-sans font-light">
              <p>{profile.aboutMe[lang]}</p>
            </div>
          </div>

          {/* Right: Education & Certifications */}
          <div className="lg:col-span-5 space-y-6 bg-zinc-900/20 border border-zinc-800/60 p-6 rounded-2xl" id="about_credentials">
            
            {/* Education */}
            <div className="space-y-4">
              <h4 className="text-xs uppercase tracking-widest text-zinc-400 font-mono font-bold flex items-center gap-2">
              ...
              {data.education.map((edu) => (
                <div key={edu.id} className="border-l-2 border-teal-500/30 pl-4 py-1 relative">
                  <span className="absolute -left-1.5 top-2.5 w-2.5 h-2.5 rounded-full bg-teal-500"></span>
                  <h5 className="text-sm font-bold text-white font-sans">{edu.degree[lang]}</h5>
                  <p className="text-xs text-zinc-400 font-sans mt-0.5">{edu.institution[lang]}</p>
                  <p className="text-[11px] text-zinc-500 font-sans mt-0.5">{edu.location[lang]}</p>
                  <p className="text-xs font-mono text-teal-400 mt-1.5">{edu.year}</p>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4 pt-6 border-t border-zinc-800/60">
              ...
              <div className="grid grid-cols-1 gap-3">
                {data.certificates.map((cert) => (
                  <div key={cert.id} className="p-3 bg-zinc-950/60 border border-zinc-850 rounded-xl flex items-start gap-2.5">
                    <div className="h-6 w-6 rounded bg-teal-500/10 flex items-center justify-center text-teal-400 shrink-0 mt-0.5">
                      <Award size={14} />
                    </div>
                    <div>
                      <h5 className="text-xs font-bold text-zinc-200 font-sans">{cert.name[lang]}</h5>
        ...
        <div id="why-me" className="mt-20 pt-16 border-t border-zinc-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" id="why_cards_grid">
            {data.whyWorkWithMe.map((item) => (
              <div key={item.id} className="bg-zinc-900/20 border border-zinc-800/60 p-5 rounded-2xl flex flex-col justify-between hover:border-teal-500/30 transition-all duration-300 group relative">
                ...
                <div className="space-y-4 relative z-10">
                  <div className="h-10 w-10 rounded-xl bg-zinc-950 border border-zinc-800 flex items-center justify-center shadow-inner group-hover:border-teal-500/20 transition-all">
                    {getWhyIcon(item.iconName)}
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-zinc-100 group-hover:text-teal-400 transition-colors font-sans">{item.title[lang]}</h4>
                    <p className="text-xs text-zinc-400 mt-2 leading-relaxed font-sans font-light">{item.description[lang]}</p>
                  </div>
                </div>
              </div>
            ))}
